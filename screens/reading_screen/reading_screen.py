...
class ReadingScreen(MDScreen):
    ...
    def on_touch_down(self, touch):
        if self.collide_point(touch.x, touch.y) and touch.y > self.height * 0.3:
    ...
    def update_definition_card_process(self, text):
        process = threading.Thread(target = self.update_definition_card, args = (text,))
        process.start()

    def update_definition_card(self, text):
        self.text_in_english = text
        definitions_info = def_api.get_definitions(text)
        Clock.schedule_once(lambda dt: self.update_definition_card_ui(definitions_info))
    ...
    def update_definition_card_ui(self, definitions_info):
        ...
        if definitions_info == -1:
            ...
            if definitions_info.get('phonetic') != '':
                ...
                scroll_view_layout.add_widget(DefinitionCardPhoneticLabel(phonetic=formated_phonetic))
            for meaning in definitions_info['meanings']:
                scroll_view_layout.add_widget(DefinitionCardPartOfSpeechLabel(part_of_speech=meaning['part_of_speech']))
                cur_definition += f'{meaning["part_of_speech"]}\n'
                cnt = 1
                for definition in meaning['definitions']:
                    scroll_view_layout.add_widget(DefinitionCardDefinitionLabel(cnt=cnt, definition_text=definition))
                    cur_definition += f'{cnt}. {definition}\n'
                    cnt += 1
        self.english_definition = cur_definition
    
    def update_translation_card_process(self, text):
        process = threading.Thread(target = self.update_translation_card, args = (text,))
        process.start()
    ...
    def prepare_cards(self, text):
        ...
        for child in delete_children:
            if child.__class__.__name__ != 'MDLabel':
                scroll_view_layout.remove_widget(child)
        scroll_view_layout.add_widget(MDLabel(text='Loading...', font_name='fonts/Helvetica', theme_text_color='Custom', text_color='black', adaptive_height=True))
    ...
    def slide_translation_card_up(self, text, active_text_input):
        ...
        anim = Animation(pos_hint={"center_x": 0.5, "center_y": 0.15}, duration=0.3)
        ...
        Clock.schedule_once(lambda dt: setattr(self.active_text_input, 'focus', False), 2)

    def slide_translation_card_down(self):
        anim = Animation(pos_hint={"center_x": 0.5, "center_y": -0.16}, duration=0.3)
        anim.start(self.translation_div.ids.translation_card)
        Clock.schedule_once(lambda dt: setattr(self.active_text_input, 'focus', False), 0)
        self.is_card_active = False

    def add_element_image_no_comment(self, element_info):
        self.reading_scroll_view.add_widget(ElementImageNoComment(image=element_info['image']))

    def add_element_image_author(self, element_info):
        self.reading_scroll_view.add_widget(ElementImageAuthor(image=element_info['image'], author=element_info['author']))

    def add_element_image_comment_author(self, element_info):
        self.reading_scroll_view.add_widget(ElementImageCommentAuthor(image=element_info['image'], author=element_info['author'], comment=element_info['comment']))

    def add_element_main_heading_comment_image_with_comment(self, element_info):
        d = str(element_info['date'].strftime('%Y-%m-%d'))
        article_date = aux.formate_date(d)
        self.reading_scroll_view.add_widget(ElementMainHeadingCommentImageWithComment(source=element_info['source'], heading_text=element_info['heading'], author=element_info['author'], date=article_date, comment=element_info['comment'], image=element_info['image'], image_source=element_info['image_source'], image_comment=element_info['image_comment']))

    def add_element_main_heading_comment_image(self, element_info):
        d = str(element_info['date'].strftime('%Y-%m-%d'))
        article_date = aux.formate_date(d)
        self.reading_scroll_view.add_widget(ElementMainHeadingCommentImage(source=element_info['source'], heading_text=element_info['heading'], author=element_info['author'], date=article_date, comment=element_info['comment'], image=element_info['image'], image_source=element_info['image_source']))
    ...
    def add_elements_process(self, article_heading):
        process = threading.Thread(target = self.get_elements_function, args = (article_heading,))
        process.start()
    ...
    def add_elements_ui(self, elements_info):
        ...
        for element_info in elements_info:
            if element_info['element_class_name'] == 'paragraph':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_paragraph(element_info), interval)
            elif element_info['element_class_name'] == 'main_heading_comment_image':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_main_heading_comment_image(element_info), interval)
            elif element_info['element_class_name'] == 'main_heading_comment_image_with_comment':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_main_heading_comment_image_with_comment(element_info), interval)
            elif element_info['element_class_name'] == 'image_comment_author':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_image_comment_author(element_info), interval)
            elif element_info['element_class_name'] == 'image_author':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_image_author(element_info), interval)
            elif element_info['element_class_name'] == 'image_no_comment':
                Clock.schedule_once(lambda dt, element_info=element_info: self.add_element_image_no_comment(element_info), interval)
            interval += 0.05
        self.translation_div = TranslationDefinitionDiv()
        Clock.schedule_once(lambda dt: self.add_widget(self.translation_div))
        Clock.schedule_once(lambda dt: self.transition_to_reading_screen(), interval + 0.4)
    ...
    def get_elements_function(self, article_heading):
        ...
        article_id = db.get_article_id_by_heading(self.db_connection, article_heading)
        article_elements = db.get_article_elements(self.db_connection, article_id)
        elements_info = []
        for element in article_elements:
            element_info = db.get_element_info(self.db_connection, element[0], element[1])
            elements_info.append(element_info)
        print("FINISHED")
        Clock.schedule_once(lambda dt: self.add_elements_ui(elements_info), 0.05)
