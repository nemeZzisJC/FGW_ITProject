...
class DialogCreateFolder(MDDialog):
    ...
    def validate_folder_name(self):
        try:
            ...
            dialog = DialogOkayButton(
                ...
                text='Folder name can only consist of small Russian or English letters, digits and "_" symbol.'
            )
            dialog.open()
        except FolderNotUnique:
            dialog = DialogOkayButton(
                title="Folder already exists!",
                text='Folder with that name already exists in current folder.'
            )
            dialog.open()

    def on_adding_folder_confimation(self, db_connection, user_id, folder_name):
        root = App.get_running_app().root
        db.add_new_folder(db_connection, self.current_folder, user_id, folder_name)
        self.confirmation_dialog.dismiss()
        root.ids.save_to_folder_word_folders_screen.add_folders_to_screen(parent_id=self.current_folder)
        root.ids.save_to_folder_word_folders_screen.add_words_to_screen(parent_id=self.current_folder)
    ...
    def add_folder(self):
        ...
        if aux.folder_name_validation(folder_name):
            if db.is_unique_folder_by_parent_id(db_connection, self.current_folder, folder_name):
                self.confirmation_dialog = MDDialog(
                    title=f'Add "{folder_name}" folder?',
                    text=f'The folder would be added to {self.path_to_folder}.',
                    buttons=[
                        DialogFlatButton(
                            text='Okay',
                            on_release=lambda x: self.on_adding_folder_confimation(db_connection, user_id, folder_name)
                        ),
                        DialogFlatButton(
                            text='Cancel',
