...
class CatalogSearch(MDScreen):
    ...
    def add_catalog_articles(self, article_ids):
        ...
        for article_id in article_ids:
            ...
            main_heading_info = db.get_main_heading_info_by_article_id(
                self.db_connection, article_id)
            if main_heading_info[0] in (
                    'main_heading_comment_image_with_comment', 'main_heading_comment_image'):
                comment = db.get_main_heading_comment_by_element_id(
                    self.db_connection, main_heading_info[0], main_heading_info[2])
    ...
    def search_bar_response(self, text):
        ...
        article_ids = db.search_bar_server(
            self.db_connection, text, root.category)
